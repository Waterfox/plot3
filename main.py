...
# !!!! I RE-ENABLED THE COLOR DICT, IF THERE IS SOME ALTERNATVE THAT I'M MISSING PLEASE INFORM
...
class MainSettingHandler(Plot3Handler):
	def get(self):
		[loggedIn,admin,UN] = self.checkCookies()
		if loggedIn: self.render('settings_main.html',colorDict=colorDict)
...
class PlotSettingHandler(Plot3Handler):
	def get(self):
		[loggedIn,admin,UN] = self.checkCookies()
		if loggedIn: self.render('settings.html',admin=loggedIn,UN=UN,colorDict=colorDict)
...
class RandomPlotSettingHandler(Plot3Handler):
	def get(self):
		self.render('settings.html',admin=True,randset=True,colorDict=colorDict) #allow any property to be changed
